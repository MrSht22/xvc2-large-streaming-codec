...
class AudioPreprocessor:
    ...
    def normalize_loudness(self, audio):
        ...
        input_peak = numpy.amax(numpy.abs(audio))
        if not numpy.isfinite(input_peak) or input_peak == 0:
            return numpy.zeros_like(audio, dtype=numpy.float32)

        # pyloudnorm requires strictly more audio than its analysis block.
        # Keep ordinary inputs unchanged and extend only very short references.
        analysis_audio = audio
        minimum_samples = int(self.meter.block_size * self.sr) + 1
        if analysis_audio.shape[0] < minimum_samples:
            padding = [(0, minimum_samples - analysis_audio.shape[0])]
            if audio.ndim > 1:
                padding.extend([(0, 0)] * (audio.ndim - 1))
            analysis_audio = numpy.pad(analysis_audio, padding, mode="constant")

        loudness = self.meter.integrated_loudness(analysis_audio)
        loud_normed = pyln.normalize.loudness(audio, loudness, -30.0)
        peak = numpy.amax(numpy.abs(loud_normed))
        if not numpy.isfinite(peak) or peak == 0:
            return numpy.zeros_like(loud_normed, dtype=numpy.float32)
